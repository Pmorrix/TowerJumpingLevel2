...
{
    [Header("Goal")]
    [SerializeField] private BuildingGoalController goalBuilding;
    ...
    [SerializeField] private float taxStartDelay = 10f;
    ...
    private Coroutine _taxStartDelayRoutine;
    private bool _goalPendingUntilPlayerLeaves;
    private bool _hasReachedNonGoalBuilding;
    ...
    public void OnExitBuildingLeft()
    {
        ...
        _hasReachedNonGoalBuilding = true;
        ...
        if (!_goalEnabled && _goalEnableRoutine == null)
            StartGoalEnableRoutine();

        if (!_timerStarted && _taxStartDelayRoutine != null)
            return;

        StartOrResumeTaxTimer();
    }
    ...
    public void OnPlayerExitedGoalArea()
    {
        ...
        if (!_hasReachedNonGoalBuilding)
            return;

        if (goalBuilding != null && goalBuilding.IsPlayerOnGoal)
            return;
    ...
    private void EnableGoalNow()
    {
        ...
        if (goalBuilding != null)
            goalBuilding.SetGoalEnabled(true);
    ...
    public void DisableGoal()
    {
        ...
        if (goalBuilding != null)
            goalBuilding.SetGoalEnabled(false);
    }

    /// <summary>
    /// Tras morir:
    /// - apaga el goal
    /// - mantiene el TAX en marcha con su valor actual
    /// - bloquea la suma de score hasta que el player salga del edificio
    /// </summary>
    public void ResetSpawnStateAfterRespawn()
    {
        ...
        DisableGoal();
        _hasReachedNonGoalBuilding = false;

        if (_timerStarted)
            StartOrResumeTaxTimer();
        else if (_taxStartDelayRoutine == null)
            StartTaxAfterConfiguredDelay();
    ...
    private void StartTaxAfterConfiguredDelay()
    {
        StopTaxStartDelayRoutine();

        float delay = Mathf.Max(0f, taxStartDelay);
        if (delay <= 0f)
        {
            StartOrResumeTaxTimer();
            return;
        ...
        _taxStartDelayRoutine = StartCoroutine(StartTaxAfterDelay(delay));
    }

    private IEnumerator StartTaxAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);

        _taxStartDelayRoutine = null;
        StartOrResumeTaxTimer();
    }

    private void StopTaxStartDelayRoutine()
    {
        if (_taxStartDelayRoutine == null)
            return;

        StopCoroutine(_taxStartDelayRoutine);
        _taxStartDelayRoutine = null;
    }

    private void StartOrResumeTaxTimer()
    {
        if (_completed || _timeUp)
            return;
